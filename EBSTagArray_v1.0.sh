...
# Script to evaluate EBS Volume Tags. Looks through all instances in account based on KEy Tag variables and for each volume on each instance the Attachment Tag is added to each volume assoicated to that instance
...
#### tag_value -- This is the value in the Key you are searching for, in this instance * selcts any value. You can be more granular if needed and look for a specific Key value, ie valuue=OS etc.
#### ec2array -- This is the array that holds InstanceID,s for the instances found from AWS describe instances request.
#### tagarray -- This is the array that holds all of the Key Tags assoicated to the volumeID and InstanceID for use in Case statement.
...
#### tagarray1 -- This is the array that holds all of the Key Tags assoicated to the volumeID for use in Case statement
...
# dryrun yes or no - uses aws cli dry-run parametre to check and not perform processing
dryrun="no"

tag_key="Name"
tag_value="*"

# Queires Instances based on Tag Key and value and stores in ec2rray
...
# Loops through ec2array and stores query results in to tagarray. Case statemnt is used to assign the correct values to varaiables for evaluation in if and else statements.
...
# Queires Volumes based on $instance and stores in volarry
...
# Loops through volarray and stores query results in to tagarray1. Case statemnt is used to assign the correct values to varaiables for evaluation in if and else statements
...
# Evalution of Instance Tag NAME v Volume Tag NAME
                                    if [ $dryrun = "yes" ]; then
...
# Evalution of Instance Tag ENVIRONMENT v Volume Tag ENVIRONMENT
                                    if [ $dryrun = "yes" ]; then
...
# Evalution of Instance Tag OS v Volume Tag OS
                                    if [ $dryrun = "yes" ]; then
...
# Evalution of Instance Tag OWNER v Volume Tag OWNER
                                    if [ $dryrun = "yes" ]; then
...
# Evalution of Instance Tag PROJECT v Volume Tag PROJECT
                                    if [ $dryrun = "yes" ]; then
...
# Evalution of Instance Tag RELEASE v Volume Tag RELEASE
                                    if [ $dryrun = "yes" ]; then
...
# Evalution of Instance Tag ROLE v Volume Tag ROLE
                                    if [ $dryrun = "yes" ]; then
...
# Evalution of Instance Tag ATTACHMENT v Volume Tag ATTACHMENT
                                    if [ $dryrun = "yes" ]; then
